  ...
  <AdminLayout>
    <div class="statistics-container">
      ...
      <el-row :gutter="20" class="chart-row">
        <el-col :span="12">
          <el-card class="chart-card">
            <template #header><span>各器材数量统计</span></template>
            <div ref="countBarRef" class="chart-container"></div>
          </el-card>
        </el-col>
        <el-col :span="12">
          <el-card class="chart-card">
            <template #header><span>热门器材 TOP10（按预约次数）</span></template>
...
const countBarRef = ref<HTMLElement>()
...
const initCharts = () => {
  ...
  // 器材数量统计柱状图
  if (countBarRef.value) {
    countBarChart = echarts.init(countBarRef.value)
    const names = statistics.equipmentCountStatistics.map(item => item.equipmentName)
    const values = statistics.equipmentCountStatistics.map(item => item.totalCount)
    countBarChart.setOption({
      tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: '{b}: {c}台' },
      grid: { left: '3%', right: '4%', bottom: '15%', containLabel: true },
      xAxis: { type: 'category', data: names, axisLabel: { rotate: 30, interval: 0 } },
      yAxis: { type: 'value', minInterval: 1, name: '数量(台)' },
      series: [{
        name: '器材数量',
        type: 'bar',
        data: values,
